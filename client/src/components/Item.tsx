...
const Item = () => {
  ...
  const [tvData, setTVData] = useState<TV[][]>([]); // TV 데이터를 저장할 상태
  useEffect(() => {
    Promise.all(
      genres.map((genre) => {
        return GetTVData()
      ...
      {genres.map((genre, index) => (
        <Genrelist key={index}>
          ...
          {tvData[index] ? (
            ...
            >
              {tvData[index]
                .filter((tv: TV) => tv.genre.includes(genre))
                .map((tv: TV, index: number) => (
                  <S_SwiperSlide key={tv.id} virtualIndex={index}>
                    <SwiperSlideImg src={tv.mainPoster} alt={`TV ${index + 1}`} />
                    <SwiperSlideTitle>{tv.title}</SwiperSlideTitle>
                  </S_SwiperSlide>
                ))}
            </S_Swiper>
          ) : (
